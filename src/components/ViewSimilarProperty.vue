              ...
              ">
                        <v-row>
                            <v-col>
                                ...
                                <template v-else>
                                    <v-btn title="Express interested" @click="showLoginInterestMessage" color="primary">Interested</v-btn>
                                </template>
                            </v-col>
                      ...
                      ">
                        ...
                        " small style="font-size: 40px; color: #3b6ef3" title="Remove from favorites" @click.stop="
                          onRemove(allSinglePropertyVisuals[0].property_id)
                        ">
                                            mdi-heart
                                        </v-icon>
                                        <v-icon v-else small style="font-size: 40px; color: black" title="Add to favorites" @click.stop="onAdd(allSinglePropertyVisuals[0].property_id)">
                                            mdi-heart-outline
                                        </v-icon>
                                    </template>
                                    <template v-else />
                                ...
                                <template v-else>
                                    <v-icon small style="font-size: 40px; color: black; z-index: 100" title="Add to favorites" @click.stop="showLoginMessage">
                                        mdi-heart-outline
                                    </v-icon>
                                </template>
                            </v-col>
    ...
    <v-container>
        <v-row>
            <v-col v-for="propertyVisual in allSinglePropertyVisuals" :key="propertyVisual.visuals_id">
                <v-img :src="'http://localhost:8002/' + propertyVisual.snapshot" aspect-ratio="1" class="grey lighten-2" height="200"></v-img>
            </v-col>
            <v-col cols="12" md="4">
                <v-img src="https://res.cloudinary.com/diued7ugb/image/upload/v1626370006/google-maps-759_1_zqu50z.png" aspect-ratio="1" class="grey lighten-2" height="200"></v-img>
            </v-col>
        </v-row>
    </v-container>
                    ...
                    <v-tabs-items v-model="tab">
                        <v-tab-item>
                            <v-card color="basil" flat>
                                <v-card-text>
                                    <!-- Features content -->
                                    <v-data-table :headers="featureHeaders" :items="allCurrentPropertyFeatures" :items-per-page="5" class="elevation-1"></v-data-table>
                                </v-card-text>
                            </v-card>
                        </v-tab-item>
                        <v-tab-item>
                            <v-card color="basil" flat>
                                <v-card-text>
                                    ...
                                    <v-data-table :headers="monthlyCostHeaders" :items="propertyMonthtyCosts" :items-per-page="5" class="elevation-1"></v-data-table>
                                </v-card-text>
                            </v-card>
                        </v-tab-item>
                        <!-- Neighborhood -->
                        ...
                        <v-tab-item>
                            <v-card color="basil" flat>
                                <v-card-text>
                                    <v-data-table :headers="priceHistoryHeaders" :items="currentPropertyPriceHistory" :items-per-page="5" class="elevation-1"></v-data-table>
                                </v-card-text>
                            </v-card>
                        </v-tab-item>
                    </v-tabs-items>
        ...
        async expressInterestInProperty() {
            try {
                ...
                if (response.data.status == 1) {
                    ...
                    const payload = {
                        "activity": `View similar Property with id ${this.$route.params.property_id}`,
                        "button_clicked": "Viewed a similar property"
                    }
