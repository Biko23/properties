    ...
    <v-container>
        <v-row>
            <v-col style="text-align: center">
                <h1>List of trusted {{allVendors[0].vendorCategory.vendor_category_name}}</h1>
            </v-col>
        </v-row>
    </v-container>
    <v-container>
        <v-row id="main-property">
            <v-col cols="12" xl="2" lg="3" md="4" sm="6" xs="12" v-for="vendor in allVendors" :key="vendor.vendor_id">
                <service-provider-card :vendor_name="vendor.vendor_name" :vendor_primary_phone_number="vendor.vendor_primary_phone_number" :vendorCategory="vendor.vendorCategory.vendor_category_name" :src="'http://localhost:8091/' + vendor.vendorCategory.snapshot" :points="vendor.points">
                    <div>
